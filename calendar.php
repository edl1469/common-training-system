...
if (!$mysql_connect) {
    ...
    $calcount   = 1;        // Keep track of the number of calendars printed.
    $date       = getdate();// Returns an associative array containing the date information of the current local time.
    $showmonths = 4;        // Select how many months should be shown.
    ...
    $html .= "<script language='javascript'> function toggle(hide, show) { "
    ...
    do {
        ...
        while ($day <= $lastday) {
            ...
            $html .= "<label for='Day_{$day}'><span class='dayNumber' "
                    ."title='{$day_name}, {$month_name} {$day}'>{$day}</span> </label>\n";
            ...
            while ($row = mysql_fetch_array($result)) {
                ...
                $html .= "<br /><br /><a href='details.php?id={$tid}' class='tt'>{$title}"
                        ...
                        ."<span class='middle'>{$desc}<br>{$time}<br>Location: {$location}<br>Trainer: {$trainer}</span>"
                        ."<span class='bottom'></span></span></a><br /><strong>{$state}</strong><br />";
            }
