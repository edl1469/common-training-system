    ...
    if (!$tid) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $registrant .= "<tr><td valign='bottom'>{$row['FirstName']}</td><td valign='bottom'>{$row['LastName']}</td><td valign='bottom'>{$row['Dept']}</td>";
                $registrant .= "<td valign='bottom'>{$row['EmpID']}</td><td><p class='sig lft'><span >&nbsp;</span></p></td></tr>";
            }
        }
        $result->free();
        ...
        $html .= "<tr><th>First</th><th>Last</th><th>Dept</th><th>Id</th><th class='lft'>Signature</th></tr>";
