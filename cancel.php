...
if ($mysqli->connect_error) {
  ...
  $message = (isset($_GET['success']))? 'You have successfully canceled that course.': 'Click on a link to cancel your registration.';
  ...
  $query = "SELECT * FROM Trainees WHERE EmpID='{$reg_empid}' AND Email='{$reg_email}' "."AND TDate>='{$today}' ORDER BY TDate ASC";
  $result = $mysqli->query($query);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      $list .= "<tr><td>{$row['Description']} ";
      $list .= ($row['Wait'])? "<span class='waiting'>( Wait-listed )</span></td>": '</td>';
      $list .= "<td>".date("l, F j, Y", strtotime($row['TDate']))." @ ".date("g:i", strtotime($row['TStartTime']))." - ".date("g:i A", strtotime($row['TEndTime']))."</td>";
  ...
  // ########## Write content
  $page['content'] = $html;
  $page['css'] = "<style type='text/css'> .waiting { float: right; font-weight: bold; margin-right: 2em; } </style>";
